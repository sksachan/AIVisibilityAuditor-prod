...
def _selected_external_pages(scope: dict) -> list[dict]:
    by_url: dict[str, dict] = {}
    for r in scope.get('queries', []):
        for p in r.get('external_pages', []):
            url = p.get('url')
...
def main() -> None:
    cfg = get_config()
    out_path = cfg['paths']['external_pages_full']
    scope = read_json(cfg['paths']['evidence_scope'])
    selected = _selected_external_pages(scope)
    selected_by_url = {p['url']: p for p in selected}

    force = bool(cfg.get('force_refetch_external_pages') or cfg.get('force_refetch_local_external'))
    existing = {'external_pages': [], 'failed_sources': []}
    ...
    existing_by_url = {p.get('url'): p for p in existing.get('external_pages', []) if p.get('url') and _is_reusable(p)}
    ...
    if not missing and cfg.get('reuse_existing_outputs', True) and not force:
        print(f'Reusing existing {out_path}; all selected external URLs already collected by local full-page scraper')
        return

    scrape_cfg = (cfg.get('scraping') or {})
    scraped = scrape_items_sync(missing if not force else selected, 'outputs/free_hybrid/external_pages', 'external', scrape_cfg)
    ...
    failed = list(existing.get('failed_sources', [])) + scraped.get('failed', [])
    ...
    out = {
        ...
        'sources_collected': sum(1 for p in pages if p.get('crawl_status') == 'success'),
        'sources_weak': sum(1 for p in pages if p.get('crawl_status') == 'weak'),
        'sources_failed': sum(1 for p in pages if p.get('crawl_status') in {'failed', 'blocked'}),
